import { AlertCircle, CalendarClock, CheckCircle2, Facebook, ImagePlus, Instagram, LayoutList, Link2, Pencil, Plus, RefreshCw, Send, ShieldCheck, Trash2, X } from "lucide-react";
...
export default function SocialPublishingAdmin() {
  ...
  return <main className="min-h-screen bg-[#080b0d] text-zinc-100"><div className="mx-auto max-w-[1440px] px-5 py-7 sm:px-8">
    ...
    {loading || !overview ? <div className="rounded-2xl border border-white/10 bg-white/[.025] p-10 text-center text-sm text-zinc-500">Carregando calendário editorial…</div> : <>
      ...
      <section className="rounded-2xl border border-white/10 bg-white/[.025] p-5"><div className="flex items-center gap-2"><LayoutList className="h-4 w-4 text-cyan-300" /><h2 className="font-['Space_Grotesk'] text-lg font-light text-white">Calendário de agendamentos</h2></div>{overview.posts.length === 0 ? <div className="py-14 text-center text-sm text-zinc-500">Nenhuma publicação criada ainda.</div> : <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{overview.posts.map((post) => <article key={post.id} className="rounded-xl border border-white/10 bg-black/10 p-4"><div className="flex items-start justify-between gap-2"><div><p className="text-sm font-medium text-white">{post.title}</p><p className="mt-1 text-[10px] uppercase tracking-[.14em] text-zinc-600">{post.unitName} · {formatLabel[post.contentFormat]}</p></div><div className="flex gap-1"><button title="Editar data" onClick={() => void editPostSchedule(post)} className="rounded border border-white/10 p-1.5 text-zinc-300"><Pencil className="h-3.5 w-3.5" /></button><button title="Excluir" onClick={() => void cancelPost(post.id)} className="rounded border border-rose-300/20 p-1.5 text-rose-200"><Trash2 className="h-3.5 w-3.5" /></button></div></div><p className="mt-3 flex items-center gap-1.5 text-xs text-zinc-400"><CalendarClock className="h-3.5 w-3.5" />{humanDate(post.scheduledFor)}</p><p className="mt-3 inline-flex rounded-full border border-white/10 px-2.5 py-1 text-[10px] text-zinc-300">{statusLabel[post.status] ?? post.status}</p></article>)}</div>}</section>
    </>}
  </div></main>;
}
