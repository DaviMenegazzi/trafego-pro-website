import { AlertCircle, CalendarClock, CheckCircle2, Facebook, ImagePlus, Instagram, LayoutList, Link2, Plus, RefreshCw, Send, ShieldCheck, X } from "lucide-react";
  ...
  return <main className="min-h-screen bg-[#080b0d] text-zinc-100"><div className="mx-auto max-w-[1440px] px-5 py-7 sm:px-8">
    ...
    {loading || !overview ? <div className="rounded-2xl border border-white/10 bg-white/[.025] p-10 text-center text-sm text-zinc-500">Carregando calendário editorial…</div> : <>
      ...
      <section className="mb-7 rounded-2xl border border-white/10 bg-white/[.025] p-5">
        ...
        {showComposer && <form onSubmit={createPost} className="mt-5 grid gap-4 border-t border-white/10 pt-5 lg:grid-cols-2"><label className="text-[10px] uppercase tracking-[.14em] text-zinc-500">Unidade<select value={form.unitId} onChange={(event) => setForm({ ...form, unitId: event.target.value, connectionId: "" })} className={fieldClass}><option value="">Selecione a unidade</option>{overview.units.map((unit) => <option key={unit.id} value={unit.id}>{unit.name}</option>)}</select></label><label className="text-[10px] uppercase tracking-[.14em] text-zinc-500">Conta Meta conectada<select value={form.connectionId} onChange={(event) => setForm({ ...form, connectionId: event.target.value })} className={fieldClass}><option value="">Ainda não conectada — manter em espera</option>{compatibleConnections.map((connection) => <option key={connection.id} value={connection.id}>{connection.facebookPageName}{connection.instagramUsername ? ` · @${connection.instagramUsername}` : ""}</option>)}</select></label><label className="text-[10px] uppercase tracking-[.14em] text-zinc-500">Título<input required value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} maxLength={255} className={fieldClass} placeholder="Ex.: Campanha de inverno" /></label><label className="text-[10px] uppercase tracking-[.14em] text-zinc-500">Formato<select value={form.contentFormat} onChange={(event) => setForm({ ...form, contentFormat: event.target.value as SocialPost["contentFormat"] })} className={fieldClass}><option value="image">Imagem</option><option value="carousel">Carrossel</option><option value="video">Vídeo</option><option value="reel">Reel</option></select></label><label className="lg:col-span-2 text-[10px] uppercase tracking-[.14em] text-zinc-500">Legenda<textarea required value={form.caption} onChange={(event) => setForm({ ...form, caption: event.target.value })} rows={4} className={fieldClass} placeholder="Escreva a legenda, hashtags e chamada para ação." /></label><label className="lg:col-span-2 text-[10px] uppercase tracking-[.14em] text-zinc-500">Mídias por URL pública<textarea required value={form.mediaUrls} onChange={(event) => setForm({ ...form, mediaUrls: event.target.value })} rows={3} className={`${fieldClass} font-mono text-xs`} placeholder="https://cdn.exemplo.com/midia.jpg" /><span className="mt-1 block normal-case tracking-normal text-zinc-600">A Meta lê mídias em URL HTTPS. Para carrossel, use uma linha por mídia.</span></label><label className="text-[10px] uppercase tracking-[.14em] text-zinc-500">Link opcional<input value={form.linkUrl} onChange={(event) => setForm({ ...form, linkUrl: event.target.value })} type="url" className={fieldClass} placeholder="https://" /></label><label className="text-[10px] uppercase tracking-[.14em] text-zinc-500">Publicar em<input value={form.scheduledFor} onChange={(event) => setForm({ ...form, scheduledFor: event.target.value })} type="datetime-local" className={fieldClass} /></label><div className="flex flex-wrap items-center gap-4 lg:col-span-2"><label className="flex items-center gap-2 text-sm text-zinc-300"><input checked={form.targetFacebook} onChange={(event) => setForm({ ...form, targetFacebook: event.target.checked })} type="checkbox" className="accent-cyan-300" /><Facebook className="h-4 w-4 text-[#7ba7ff]" />Página Facebook</label><label className="flex items-center gap-2 text-sm text-zinc-300"><input checked={form.targetInstagram} onChange={(event) => setForm({ ...form, targetInstagram: event.target.checked })} type="checkbox" className="accent-cyan-300" /><Instagram className="h-4 w-4 text-pink-300" />Instagram profissional</label><button disabled={creating} className="ml-auto inline-flex items-center gap-2 rounded-lg bg-cyan-300 px-4 py-2 text-xs font-semibold text-[#062428] disabled:opacity-60"><Send className="h-3.5 w-3.5" />{creating ? "Salvando…" : form.scheduledFor ? "Agendar publicação" : "Salvar rascunho"}</button></div></form>}
      </section>

      {showBulkPlanner && <section className="mb-7 overflow-hidden rounded-2xl border border-cyan-300/20 bg-cyan-300/[.035]"><div className="flex flex-col gap-3 border-b border-cyan-300/15 p-5 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-[10px] font-medium uppercase tracking-[.2em] text-cyan-300">Fila mensal persistente</p><h2 className="mt-1 font-['Space_Grotesk'] text-xl font-light text-white">Planejar conteúdo em massa</h2><p className="mt-1 max-w-3xl text-xs leading-5 text-zinc-400">As linhas ficam salvas neste navegador mesmo ao recarregar. Revise unidade, mídia e horário; o sistema envia no máximo dez peças por vez e evita duplicação se a página for interrompida.</p></div><div className="flex flex-wrap gap-2"><button onClick={() => addBulkItem()} className="rounded-lg border border-white/10 px-3 py-2 text-xs text-zinc-200"><Plus className="mr-1 inline h-3.5 w-3.5" />Adicionar linha</button><button disabled={bulkSending} onClick={() => void sendBulkQueue()} className="rounded-lg bg-cyan-300 px-3 py-2 text-xs font-semibold text-[#062428] disabled:opacity-60"><Send className="mr-1 inline h-3.5 w-3.5" />{bulkSending ? "Enviando…" : "Enviar fila"}</button></div></div>{bulkSummary && <p className="border-b border-cyan-300/15 px-5 py-3 text-xs text-cyan-100">{bulkSummary}</p>}<div className="overflow-x-auto"><div className="min-w-[1260px]"><div className="grid grid-cols-[150px_160px_190px_140px_220px_230px_190px_110px] gap-2 border-b border-white/10 bg-black/10 px-5 py-2 text-[10px] uppercase tracking-[.12em] text-zinc-500"><span>Unidade</span><span>Conta Meta</span><span>Título e formato</span><span>Canais</span><span>Legenda</span><span>Mídias HTTPS</span><span>Data e horário</span><span>Ação</span></div>{bulkQueue.length === 0 ? <div className="px-5 py-10 text-center text-sm text-zinc-500">Comece adicionando as peças do mês. O rascunho fica salvo neste navegador até ser enviado.</div> : bulkQueue.map((item) => <div key={item.localId} className={`grid grid-cols-[150px_160px_190px_140px_220px_230px_190px_110px] gap-2 border-b border-white/8 px-5 py-3 text-xs ${item.state === "error" ? "bg-rose-400/[.06]" : ""}`}><select value={item.unitId} onChange={(event) => updateBulkItem(item.localId, { unitId: event.target.value, connectionId: "" })} className={compactFieldClass}><option value="">Unidade</option>{overview.units.map((unit) => <option key={unit.id} value={unit.id}>{unit.name}</option>)}</select><select value={item.connectionId} onChange={(event) => updateBulkItem(item.localId, { connectionId: event.target.value })} className={compactFieldClass}><option value="">Em espera</option>{overview.connections.filter((connection) => connection.unitId === item.unitId && connection.connectionStatus === "active").map((connection) => <option key={connection.id} value={connection.id}>{connection.facebookPageName}</option>)}</select><div className="space-y-1"><input value={item.title} onChange={(event) => updateBulkItem(item.localId, { title: event.target.value })} className={compactFieldClass} placeholder="Título" /><select value={item.contentFormat} onChange={(event) => updateBulkItem(item.localId, { contentFormat: event.target.value as BulkQueueItem["contentFormat"] })} className={compactFieldClass}><option value="image">Imagem</option><option value="carousel">Carrossel</option><option value="video">Vídeo</option><option value="reel">Reel</option></select></div><div className="space-y-1.5 py-1 text-zinc-300"><label className="flex items-center gap-1.5"><input checked={item.targetFacebook} onChange={(event) => updateBulkItem(item.localId, { targetFacebook: event.target.checked })} type="checkbox" className="accent-cyan-300" /><Facebook className="h-3.5 w-3.5 text-[#7ba7ff]" />Facebook</label><label className="flex items-center gap-1.5"><input checked={item.targetInstagram} onChange={(event) => updateBulkItem(item.localId, { targetInstagram: event.target.checked })} type="checkbox" className="accent-cyan-300" /><Instagram className="h-3.5 w-3.5 text-pink-300" />Instagram</label></div><textarea value={item.caption} onChange={(event) => updateBulkItem(item.localId, { caption: event.target.value })} rows={3} className={compactFieldClass} placeholder="Legenda" /><textarea value={item.mediaUrls} onChange={(event) => updateBulkItem(item.localId, { mediaUrls: event.target.value })} rows={3} className={`${compactFieldClass} font-mono text-[10px]`} placeholder="https://…" /><div className="space-y-1"><input value={item.scheduledFor} onChange={(event) => updateBulkItem(item.localId, { scheduledFor: event.target.value })} type="datetime-local" className={compactFieldClass} />{item.error && <p className="text-[10px] leading-4 text-rose-200">{item.error}</p>}{item.state === "saved" && <p className="text-[10px] text-emerald-200">No calendário</p>}</div><div className="flex items-start gap-1"><button title="Duplicar linha" onClick={() => addBulkItem(item)} className="rounded border border-white/10 p-1.5 text-zinc-300"><Plus className="h-3.5 w-3.5" /></button><button title="Remover linha" onClick={() => setBulkQueue((items) => items.filter((value) => value.localId !== item.localId))} className="rounded border border-white/10 p-1.5 text-zinc-300"><X className="h-3.5 w-3.5" /></button></div></div>)}</div></div></section>}
